...
A property listed here is NOT a security guarantee. It is a claim that can be inspected, challenged, and attacked. The status ladder records what evidence actually exists for each property. If the evidence is thin, the status says so.
...
Claim: A granted AuthorityTransition evaluated against authorityVersion = N cannot be committed into a KernelState whose authorityVersion is any value other than N, regardless of whether its transition ID has been committed before.
